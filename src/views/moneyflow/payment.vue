  ...
  <el-container>
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <el-form-item label="已付金额" :label-width="formLabelWidth" prop="paid">
...
<script>
import { getPayments, deletePayment, createPayment, updatePayment } from '@/api/payment'
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        paid: [{ message: '请输入数字！', trigger: 'blur', validator: validateData }]
      },
      listQuery: {
        page: 1,
        limit: 20,
      ...
      tHeader: ['付款日期', '经办人', '货品名称', '已付金额', '结余', '付款单位', '付款金额', '备注'],
      isDate: false,
      options: [{
        value: 'date',
        label: '付款日期'
    ...
    updateData() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          updatePayment(tempData.id, tempData).then(() => {
            this.getList()
            this.dialogFormVisible = false
            this.$message({
              type: 'success',
