...
<script>
import { getRefunds, deleteRefund, createRefund, updateRefund } from '@/api/return'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      getRefunds(this.listQuery).then(response => {
    ...
    handleDelete(row, index) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        deleteRefund(row.id).then(response => {
          this.getList()
          setTimeout(() => {
            this.listLoading = false
          }, 1.5 * 1000)
    ...
    updateData() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          updateRefund(tempData.id, tempData).then(() => {
            this.getList()
            this.dialogFormVisible = false
            this.$message({
              type: 'success',
